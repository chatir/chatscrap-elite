...
# --- 1. CONFIG & ELITE DESIGN (استرجاع التصميم الأصلي) ---
...
# --- 2. DRIVER SYSTEM (SMART AUTO-DETECT) ---
# هذا الكود كيقلب على الدرايفر فين ما كان فالسيرفر
...
def get_driver():
    ...
    if not driver_path:
        # محاولة أخيرة: مسار Streamlit الافتراضي
        if os.path.exists("/usr/lib/chromium-browser/chromedriver"):
            driver_path = "/usr/lib/chromium-browser/chromedriver"
        else:
            st.error("❌ Critical: Could not find 'chromedriver' on system path.")
            return None
    ...
    with t1:
        ...
        if st.session_state.running:
            ...
            if driver:
                ...
                st.error("❌ Driver Initialization Failed. Please refresh.")
                st.session_state.running = False
