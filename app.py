...
if st.session_state["authentication_status"]:
    ...
    # --- CSS (ORANGE ELITE DESIGN + ANIMATED BAR) ---
    orange_c = "#FF8C00"
    st.markdown(f"""
        <style>
    ...
    # ---------------------------
        ...
        with st.container():
            ...
            limit, scrolls = col3.number_input("Target Leads", 1, 2000, 20), col4.number_input("Depth", 5, 500, 30)
        ...
        with t1:
                    ...
                    try:
                        ...
                        for idx, link in enumerate(links):
                            ...
                            try:
                                ...
                                email = fetch_email(driver, website) if w_email else "N/A"

                                results.append({"Name": name, "Phone": phone, "WhatsApp": wa_link, "Website": website, "Address": adr, "Email": email})
                                ...
                                run_query("INSERT INTO leads (session_id, name, phone, website, address, whatsapp, email) VALUES (?, ?, ?, ?, ?, ?, ?)", (s_id, name, phone, website, adr, wa_link, email))
                            except: continue
            ...
            if st.session_state.results_df is not None:
                ...
                if w_sync:
                    ...
                    gs_url = st.text_input("Paste Google Sheet URL here")
                    if st.button("🚀 Sync to Sheet Now", type="primary"):
                        if gs_url:
                            if sync_to_gsheet(st.session_state.results_df, gs_url): st.success("✅ Success!")
                        else: st.warning("Provide URL first")
        ...
        with t3:
            st.subheader("🤖 Marketing Kit Generator")
            offer = st.selectbox("Service Type", ["Web Design", "SEO", "Ads Management"])
            if st.button("✨ Generate Script"):
                st.code(f"Hi! We saw your business in {city} and noticed your website could use an upgrade...", language="text")
