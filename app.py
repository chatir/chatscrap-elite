...
if st.session_state["authentication_status"] is False:
    st.error('❌ Login Incorrect'); st.stop()
...
# ==============================================================================
# 4. SCRAPING ENGINE (ANTI-BLOCK V2)
# ==============================================================================
def get_driver_beast():
    opts = Options()
    # 🔥 Robust settings for Streamlit Cloud
...
with st.sidebar:
    ...
    if is_admin:
        with st.expander("🛠️ ADMIN DASHBOARD"):
            ...
            c1, c2 = st.columns(2)
            if c1.button("💰 +100"): add_credits(tgt_usr, 100); st.rerun()
            if c2.button("🗑️ Del"): 
                run_query("DELETE FROM user_credits WHERE username=?", (tgt_usr,)); st.rerun()
...
with st.container():
    ...
    with cb:
        st.write("")
        b1, b2 = st.columns(2)
        if b1.button("START ENGINE", type="primary", use_container_width=True): # Removed use_container_width to fix log warning
            ...
            else: st.error("Enter keywords & city!")
        
        if b2.button("STOP", type="secondary", use_container_width=True): # Removed use_container_width
...
with t1:
    ...
    if st.session_state.running:
        ...
        total = len(kw_list) * len(ct_list); current = 0
        all_res = []

        run_query("INSERT INTO sessions (query, date) VALUES (?, ?)", (f"{st.session_state.p_kw} - {st.session_state.p_city}", time.strftime("%Y-%m-%d %H:%M")))
            ...
            try:
                for city in ct_list:
                    for kw in kw_list:
                        ...
                        current += 1
                        st.session_state.status_txt = f"SEARCHING: {kw} in {city} ({current}/{total})"
                        st.session_state.progress_val = int(((current-1)/total)*100)
                        st.rerun()

                        url = f"https://www.google.com/maps/search/{quote(kw)}+in+{quote(city)}"
                        driver.get(url); time.sleep(5) # Increased wait time
                        ...
                        if not unique: 
                            st.toast(f"No results for {kw} in {city}. Google might be blocking.", icon="⚠️")
