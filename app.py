...
# Persistence: Initialize inputs in session state to prevent reset
if 'niche_input' not in st.session_state: st.session_state.niche_input = ""
if 'city_input' not in st.session_state: st.session_state.city_input = ""
if 'limit_input' not in st.session_state: st.session_state.limit_input = 20
if 'scroll_input' not in st.session_state: st.session_state.scroll_input = 30
...
if st.session_state["authentication_status"]:
    ...
    def sync_to_gsheet(df, url):
        try:
            ...
            # 🔥 Fix for Sync Error: Ensure secrets are set
    ...
    def deduct_credit(username):
        if username != "admin": # 🔥 Admin is Unlimited
    ...
    with st.sidebar:
        ...
        st.divider()
        # 🔥 Logout Fix: Force rerun after logout
        if st.button("Logout", type="secondary", use_container_width=True):
            authenticator.logout('Logout', 'main')
            st.session_state.clear()
            st.rerun()

    # --- CSS (ORANGE GLOW DESIGN) ---
    orange_c = "#FF8C00"
    st.markdown(f"""
        <style>
    ...
    # ---------------------------
        ...
        # Fresh driver (No Cache to prevent MaxRetryError)
        def get_driver_live():
            opts = Options()
        ...
        # Persistence: Link widgets to session state
        with st.container():
            col1, col2, col3, col4 = st.columns([3, 3, 1.5, 1.5])
            niche = col1.text_input("🔍 Business Niche", value=st.session_state.niche_input, key="n_key")
            city = col2.text_input("🌍 Global City", value=st.session_state.city_input, key="c_key")
            limit = col3.number_input("Target", 1, 2000, value=st.session_state.limit_input, key="l_key")
            depth = col4.number_input("Depth", 5, 500, value=st.session_state.scroll_input, key="d_key")
            
            # Sync inputs to state
            st.session_state.niche_input, st.session_state.city_input = niche, city
            st.session_state.limit_input, st.session_state.scroll_input = limit, depth
        ...
        with t1:
            ...
            if st.session_state.results_df is not None:
                ...
                st.divider()
                if w_sync:
                    gs_url = st.text_input("Google Sheet URL for Sync", key="gs_sync_url")
                ...
                if driver:
                    try:
                        ...
                        for i in range(depth):
                            ...
                            time.sleep(1.2); update_bar(10 + int((i/depth)*40), "SCROLLING...")
        ...
        with t2:
            ...
            for h in hists:
                with st.expander(f"📦 {h[2]} | {h[1]}"):
                    ...
                    st.dataframe(pd.DataFrame(dat, columns=["Name", "Phone", "WhatsApp", "Website", "Address"]), use_container_width=True, column_config={"WhatsApp": st.column_config.LinkColumn("Chat", display_text="💬")})

        with t3:
            st.subheader("🤖 Marketing AI")
            offer = st.selectbox("Select Service", ["Web Design", "SEO", "Ads"])
