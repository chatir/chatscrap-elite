...
# ==============================================================================
# 2. DESIGN SYSTEM (MASTERPIECE CSS)
# ==============================================================================
...
st.markdown(f"""
    ...
    /* 🔥 RESTORING COLUMN SPACING FOR INPUTS */
    /* We only target the buttons container for Zero-Gap */
    ...
    /* 🔥 THE ANIMATED PROGRESS BAR (FIXED) */
...
# ==============================================================================
# 7. INPUTS & THE "SOLID 50/50" CONTROL BAR
# ==============================================================================
with st.container():
    # Spacing is back here because columns outside the special CSS maintain defaults
    c1, c2, c3, c4 = st.columns([3, 3, 2, 1.5])
    kw_in = c1.text_input("Keywords", placeholder="e.g. restaurant, cafe")
    ...
    st.write("")
    # 🔥 THE TRULY ATTACHED 50/50 BAR (CSS TARGETED)
    ...
    with btn_col1:
        if st.button("Start Extraction", type="primary"):
            if kw_in and city_in:
                ...
                st.session_state.progress = 0 # REAL RESET
                with sqlite3.connect(DB_NAME) as conn:
                    cur = conn.cursor()
        ...
        try:
            ...
            for city in cts:
                for kw in kws:
                    ...
                    curr_op += 1
                    # 🔥 PROGRESS CALCULATION (Starts from 0 and fills slowly)
                    st.session_state.progress = int((curr_op / total_ops) * 100)
                    prog_spot.markdown(f'<div class="prog-container"><div class="prog-bar-fill" style="width: {st.session_state.progress}%;"></div></div>', unsafe_allow_html=True)
                    status_ui.markdown(f"**Scanning:** `{kw}` in `{city}`...")
                    
                    gl = {"Morocco":"ma", "France":"fr", "USA":"us", "Spain":"es"}.get(country_in, "ma")
                    ...
                    for item in items:
                        ...
                        try:
                            ...
                            # 🔥 PRO WHATSAPP WITH ICON AND CHAT NOW
                            wa_link = "N/A"
                            cp = re.sub(r'\D', '', phone)
                            if any(cp.startswith(x) for x in ['2126','2127','06','07']) and not (cp.startswith('2125') or cp.startswith('05')):
                                wa_link = f'<a href="https://wa.me/{cp}" target="_blank" style="color:#25D366; text-decoration:none; font-weight:bold;"><i class="fab fa-whatsapp"></i> Chat Now</a>'
...
with tab_archive:
    ...
    search_f = st.text_input("Filter History", placeholder="🔍 Search e.g. 'cafe' or 'rabat'...")
        ...
        for _, sess in df_s.iterrows():
            with st.expander(f"📦 {sess['date']} | {sess['query']}"):
                ...
                if not df_l.empty:
                    ...
                    st.download_button("📥 Export CSV", df_l.to_csv(index=False).encode('utf-8-sig'), f"search_{sess['id']}.csv", key=f"dl_{sess['id']}")
