                ...
                for i, (city, kw) in enumerate(all_tasks):
                    ...
                    for item in items:
                        ...
                        try:
                            ...
                            if w_neg and rating != "N/A" and float(rating) >= 3.5: continue
        ...
        for _, sess in df_s.iterrows():
            with st.expander(f"📦 {sess['date']} | {sess['query']}"):
                ...
                if not df_l.empty:
                    ...
                    st.download_button(label="⬇️ Download CSV", data=df_l.to_csv(index=False).encode('utf-8'), file_name=f"archive_{sess['id']}.csv", key=f"btn_{sess['id']}")
